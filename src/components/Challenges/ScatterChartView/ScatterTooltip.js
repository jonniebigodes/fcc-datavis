...
const showInfo = () => {
  return (
    <p>
      <span className={styles.scattertoolTipText}>
        {' '}
...
const showData = value => {
  return (
    <p>
      <span className={styles.scattertoolTipText}>
        {value.Name} from {value.Nationality} with a time of {value.Time}{' '}
        finished in {value.Place}th place.{value.Doping === 'No Allegations'
          ? `Clean as a whistle`
          : `With doping alegations`}
      </span>
    </p>
  )
}
