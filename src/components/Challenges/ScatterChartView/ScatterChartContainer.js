...
class ScatterChartContainer extends Component {
  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      const storedScatterdata = JSON.parse(
        Utilities.getStorageData('scatterData')
      )
      ...
      .then(result => {
        ...
        const dataToStore = result.map(item => {
          return {
            ...
            Nationality: item.Nationality,
            Doping:
              item.Doping === '' ? 'No Allegations' : 'Doping Allegations',
            URL: item.URL,
      ...
      return (
        <div
          ...
          <div className={styles.containerScatter}>
            ...
            <div>
              <ScatterTooltip
                data={isTooltipActive ? tooltipData.data : null}
              />
            </div>
          </div>
