...
class DataVisForceGraph extends Component {
  constructor(props) {
    ...
    this.forceSim = forceSimulation()
      ...
      .force(
        'charge',
        forceManyBody().strength(this.setsimForce(this.props.width))
      )
  ...
  componentDidMount() {
    const nodeElements = d3selectAll('.node')
    const lineElement = d3selectAll('.link')
    this.forceSim.nodes(this.props.graphData.nodes).on('tick', () => {
      nodeElements.attr('cx', node => {
        return node.x
      })
      nodeElements.attr('cy', node => {
        return node.y
      })
      lineElement.attr('x1', line => {
        return line.source.x
      })
      lineElement.attr('y1', line => {
        return line.source.y
      })
      lineElement.attr('x2', line => {
        return line.target.x
      })
      lineElement.attr('y2', line => {
        return line.target.y
      })
    })
  }
